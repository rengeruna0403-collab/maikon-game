 ...
 */
window._MAIKON_QA_VERSION = '2026-08-17-v12b3-menu-impact-card-clarity';
...
function qa2cSwitchTab(tid,idx){
  ...
  const _sim5AdDiag = {
    ...
    insufficientCash: 0, insufficientAP: 0,
    noAvailableAd: 0, cooldownBlocked: 0,
    attempted: 0, succeeded: 0, adPurchased: 0,
    adCostTotal: 0, // v0.5i: 実際に支出された広告費の合計
    reset() {
      this.called = 0; this.noStore = 0; this.noOpenStore = 0;
      this.insufficientCash = 0; this.insufficientAP = 0;
      this.noAvailableAd = 0; this.cooldownBlocked = 0;
      this.attempted = 0; this.succeeded = 0; this.adPurchased = 0;
      this.adCostTotal = 0;
    },
  };
  window._sim5AdDiag = _sim5AdDiag;

  // v0.5i: 広告ポリシー比較用の QA 専用状態
  // _sim5AdCurrentPolicy が null のとき従来動作（クールダウンなし）
  let _sim5AdCurrentPolicy = null; // { cooldownDays: number|null, enabled: boolean }
  let _sim5AdCurrentDay    = 0;    // シミュレーションループの現在日（d と同期）
  const _sim5AdPolicyState = { lastPurchaseDayByStore: {} };

  // _adOpts: テスト時のみ AD_OPTIONS 代替を渡す。本番は省略（undefined）
  function _sim5TryBuyAd(_adOpts) {
    _sim5AdDiag.called++;
    ...
    // 広告オプション取得（テスト時は _adOpts を使用、本番は AD_OPTIONS グローバル）
    const adOptions = _adOpts !== undefined ? _adOpts : (typeof AD_OPTIONS !== 'undefined' ? AD_OPTIONS : []);
    if (adOptions.length === 0) { _sim5AdDiag.noAvailableAd++; return false; }

    // v0.5i: クールダウン判定（_sim5AdCurrentPolicy が設定されている場合のみ）
    const _pol = _sim5AdCurrentPolicy;
    if (_pol && (_pol.cooldownDays ?? 0) > 0) {
      const lastDay = _sim5AdPolicyState.lastPurchaseDayByStore[si] ?? -Infinity;
      if (_sim5AdCurrentDay - lastDay < _pol.cooldownDays) {
        _sim5AdDiag.cooldownBlocked++;
        return false;
      }
    }
    ...
    const _thisCost = opt.cost;

    try {
      buyAd(si, opt.name, opt.cost, opt.cust);
      _sim5AdDiag.adCostTotal += _thisCost; // 支出確定後に積算
      // v0.5i: 購入成功後にクールダウン状態を更新
      if (_pol && (_pol.cooldownDays ?? 0) > 0) {
        _sim5AdPolicyState.lastPurchaseDayByStore[si] = _sim5AdCurrentDay;
      }
    } catch(e) { return false; }
  ...
  // ── v0.7: buyRenov AI ─────────────────────────────────────────
  // 呼び出し順注意: investRegion→trainStaff→buyMenu→buyAd→buyRenovの順。
  // 先行行動が資金・APを消費するため、buyRenovの発火率は呼び出し順に依存する。
  // RENOV_OPTIONS価格帯: 150,000〜500,000円。最安値150,000円＋余裕分で200,000円をリザーブに設定。
  const _SIM7_RENOV_CASH_RESERVE = 200_000;

  const _sim7RenovDiag = {
    called: 0,
    noStore: 0,
    noOpenStore: 0,
    noAvailableRenov: 0,
  ...
  function _sim7TryBuyRenov(renovOptionsOverride) {
    _sim7RenovDiag.called++;

    // 注意: ローカル変数をGと命名するとグローバルGをシャドウするため _g7r を使用する
    let _g7r;
    try { _g7r = eval('G'); } catch(e) { return false; }
    if (!_g7r || !Array.isArray(_g7r.stores) || _g7r.stores.length === 0) {
      _sim7RenovDiag.noStore++;
      return false;
    }

    // 開店済みの最初の店舗を対象とする
    const si = _g7r.stores.findIndex(s => s.isOpen);
    if (si < 0) { _sim7RenovDiag.noOpenStore++; return false; }
    const store = _g7r.stores[si];

    // 未購入の改装オプションを列挙（renovOptionsOverrideが渡された場合はそちらを使用）
    const renovOptions = (renovOptionsOverride !== undefined ? renovOptionsOverride : (typeof RENOV_OPTIONS !== 'undefined' ? RENOV_OPTIONS : []));
    const upgrades = store.upgrades || [];
    const available = renovOptions.filter(r => !upgrades.includes(r.name));

    if (available.length === 0) {
      _sim7RenovDiag.allPurchased++;
      return false;
    }

    // 最も安い未購入改装を選択（同額の場合は定義順優先）
    const opt = available.reduce((best, r) => r.cost < best.cost ? r : best, available[0]);

    // 資金チェック
    if ((_g7r.money ?? 0) < opt.cost + _SIM7_RENOV_CASH_RESERVE) {
      _sim7RenovDiag.insufficientCash++;
      return false;
    }

    // APコストチェック（スタッフ配置済みなら0、なければ20）
    const hasStaff = (_g7r.staff || []).some(s => s.store === store.name);
    const apCost   = hasStaff ? 0 : 20;
    if (apCost > 0 && (_g7r.ap ?? 0) < apCost) {
      _sim7RenovDiag.insufficientAP++;
      return false;
    }

    const upgradesLenBefore = (store.upgrades || []).length;
    _sim7RenovDiag.attempted++;

    try {
      buyRenov(si, opt.name, opt.cost, opt.cust || 0, opt.sat || 0, opt.score || 0);
    } catch(e) { return false; }

    // 実際にupgradesが増加したか確認（success detection）
    let _g7rAfter;
    try { _g7rAfter = eval('G'); } catch(e) { _g7rAfter = null; }
    const upgradesLenAfter = (_g7rAfter?.stores?.[si]?.upgrades?.length ?? upgradesLenBefore);
  ...
  async function _sim8RunBuyMenuLedgerAudit({ seeds, trialsPerSeed } = {}) {
    ...
    let _gLedger;
    try { _gLedger = eval('G'); } catch(e) { return null; }
    let gSnap;
    try { gSnap = _sim3MakeFreshStartSnap(_gLedger); } catch(e) { return null; }
  ...
  const _SIM5_ACTION_DEFS = [
    ...
    },
    {
      ...
      label:           '店舗改装',
      diag:            () => _sim7RenovDiag,
      otherBlockedKeys: ['noStore', 'noOpenStore', 'noAvailableRenov', 'allPurchased'],
    },
  ];
  ...
  const _SIM5_ENABLE = {
    ...
    buyRenov:     false, // v0.7: 効果検証中 → デフォルトOFF
  };
  ...
  // ── v0.5g: AI行動別 限界効果分析 ────────────────────────────────
  const _SIM5_IMPACT_SCENARIOS = [
    { key: 'allOff',           label: '全AIなし',       enable: { investRegion:false, trainStaff:false, buyMenu:false, buyAd:false, buyRenov:false } },
    { key: 'investRegionOnly', label: '地域活動のみ',   enable: { investRegion:true,  trainStaff:false, buyMenu:false, buyAd:false, buyRenov:false } },
    { key: 'trainStaffOnly',   label: '研修のみ',       enable: { investRegion:false, trainStaff:true,  buyMenu:false, buyAd:false, buyRenov:false } },
    { key: 'buyMenuOnly',      label: 'メニュー追加のみ', enable: { investRegion:false, trainStaff:false, buyMenu:true,  buyAd:false, buyRenov:false } },
    { key: 'buyAdOnly',        label: '広告のみ',       enable: { investRegion:false, trainStaff:false, buyMenu:false, buyAd:true,  buyRenov:false } },
    { key: 'buyRenovOnly',     label: '改装のみ',       enable: { investRegion:false, trainStaff:false, buyMenu:false, buyAd:false, buyRenov:true  } },
    { key: 'allOn',            label: '全AIあり',       enable: { investRegion:true,  trainStaff:true,  buyMenu:true,  buyAd:true,  buyRenov:true  } },
  ];
  window._SIM5_IMPACT_SCENARIOS = _SIM5_IMPACT_SCENARIOS;

  function _sim5RunMarginalImpactAnalysis(numTrials, seed) {
    numTrials = numTrials || 3;
    seed      = seed      || 1001;

    // 現在のGとフレッシュスナップ
    let _gMia;
    try { _gMia = eval('G'); } catch(e) { return null; }
    const realGStr = JSON.stringify(_gMia);
    let gSnap;
    try { gSnap = _sim3MakeFreshStartSnap(_gMia); } catch(e) { return null; }
    ...
    const runScenario = (enable) => {
      ...
      return {
        ...
        minCash:         g('minCash'),
        deficitMonths:   g('deficitMonths'),
        bankruptcyRate:  trials.filter(t => (t.minCash ?? 0) < 0).length / trials.length,
        daysNoAction:    ekpi?.daysNoAction ?? null,
        investAttempted: _sim5Diag.attempted,
        trainAttempted:  _sim5TrainDiag.attempted,
        menuAttempted:   _sim5MenuDiag.attempted,
        adAttempted:     _sim5AdDiag.attempted,
        renovAttempted:  _sim7RenovDiag.attempted,
      };
    };
    ...
    try {
      // 各シナリオを順番に実行
      const results = [];
      for (const sc of _SIM5_IMPACT_SCENARIOS) {
        const metrics = runScenario(sc.enable);
        if (!metrics) return null;
        results.push({ key: sc.key, label: sc.label, enable: sc.enable, metrics });
      }

      // baseline = allOff
      const baseline = results.find(r => r.key === 'allOff');
      if (!baseline) return null;
      const bm = baseline.metrics;

      // 各シナリオに diffFromBaseline と費用対効果指標を付与
      const actionCountKey = {
        investRegionOnly: 'investAttempted',
        trainStaffOnly:   'trainAttempted',
        buyMenuOnly:      'menuAttempted',
        buyAdOnly:        'adAttempted',
        buyRenovOnly:     'renovAttempted',
      };
      const scenarios = results.map(r => {
        const dm = r.metrics;
        const diff = {
          profit:        dm.profit       - bm.profit,
          revenue:       dm.revenue      - bm.revenue,
          endCash:       dm.endCash      - bm.endCash,
          minCash:       dm.minCash      - bm.minCash,
          deficitMonths: dm.deficitMonths - bm.deficitMonths,
          bankruptcyRate:dm.bankruptcyRate - bm.bankruptcyRate,
          daysNoAction:  dm.daysNoAction - bm.daysNoAction,
        };
        const acKey   = actionCountKey[r.key];
        const acCount = acKey ? (dm[acKey] || 0) : 0;
        const costEff = acKey ? {
          incrementalProfitPerAction:  safe(diff.profit,  acCount),
          incrementalRevenuePerAction: safe(diff.revenue, acCount),
          actionCount: acCount,
        } : null;
        return { key: r.key, label: r.label, metrics: dm, diffFromBaseline: diff, costEfficiency: costEff };
      });

      // ランキング（allOff/allOn を除く単独AI4種を利益差分降順）
      const rankTargets = scenarios.filter(s => !['allOff','allOn'].includes(s.key));
      rankTargets.sort((a, b) => b.diffFromBaseline.profit - a.diffFromBaseline.profit);
      const rankings = rankTargets.map((s, i) => ({
        rank:       i + 1,
        key:        s.key,
        label:      s.label,
        profitDiff: s.diffFromBaseline.profit,
        revenueDiff:s.diffFromBaseline.revenue,
      }));
      ...
      // コンソール表示
      {
        ...
        scenarios.forEach(s => {
          const m = s.metrics;
          const d = s.diffFromBaseline;
          const ce = s.costEfficiency;
          tableData[s.label] = {
            '利益':              fmt0(m.profit),
            '利益差分':          sign(d?.profit),
            '売上':              fmt0(m.revenue),
            ...
            '年末現金':          fmt0(m.endCash),
            '最低現金':          fmt0(m.minCash),
            '赤字月数':          fmt0(m.deficitMonths),
            '無行動日数':        fmt0(m.daysNoAction),
            '行動回数':          (m.investAttempted+m.trainAttempted+m.menuAttempted+m.adAttempted+(m.renovAttempted||0)),
            '1行動あたり利益差分': ce ? Math.round(ce.incrementalProfitPerAction).toLocaleString() : '-',
            '1行動あたり売上差分': ce ? Math.round(ce.incrementalRevenuePerAction).toLocaleString() : '-',
          };
        });
        console.group('📊 AI行動別 限界効果');
        console.table(tableData);
        console.group('🏆 利益差分ランキング（単独AI）');
        rankings.forEach(r => console.log(`${r.rank}. ${r.label}: 利益差分 ${Math.round(r.profitDiff).toLocaleString()} / 売上差分 ${Math.round(r.revenueDiff).toLocaleString()}`));
        console.groupEnd();
        console.groupEnd();
      }

      return { baselineKey: 'allOff', scenarios, rankings, numTrials, seed };
  ...
  // ── v0.5h/v0.5i 共通: 統計計算ヘルパー ──────────────────────────
  function _sim5CalcStats(arr) {
    ...
    const stddev = Math.sqrt(arr.reduce((s, v) => s + (v - mean) ** 2, 0) / n);
    return { mean, median, min, max, stddev };
  }
  window._sim5CalcStats = _sim5CalcStats;
  ...
  function _sim5RunStabilityAnalysis({ seeds, trialsPerSeed } = {}) {
    ...
    const scenarios = scenarioKeys.map(key => {
      ...
      const profit = _sim5CalcStats(raw.profitDiffs);
      const revenue= _sim5CalcStats(raw.revenueDiffs);
      const endCash= _sim5CalcStats(raw.endCashDiffs);
  ...
  // ── v0.5l: デフォルトAI構成 比較 ────────────────────────────────
  const _SIM5_DEFAULT_CONFIG_SCENARIOS = [
    {
      key: 'allOff',
      label: '全AIなし',
      enable: { investRegion: false, trainStaff: false, buyMenu: false, buyAd: false, buyRenov: false },
    },
    {
      key: 'oldDefault',
      label: '旧デフォルト（広告ON）',
      enable: { investRegion: true, trainStaff: true, buyMenu: true, buyAd: true, buyRenov: false },
    },
    {
      key: 'buyMenuOnlyDefault',
      label: '本番デフォルト（buyMenuのみ）',
      enable: { investRegion: false, trainStaff: false, buyMenu: true, buyAd: false, buyRenov: false },
    },
    {
      key: 'menuOnly',
  ...
  window._SIM5_DEFAULT_CONFIG_SCENARIOS = _SIM5_DEFAULT_CONFIG_SCENARIOS;

  const _SIM5_DEFAULT_CONFIG_VALIDATION_DEFAULTS = {
    seeds: [1001,2001,3001,4001,5001,6001,7001,8001,9001,10001],
    trialsPerSeed: 10,
  };
  ...
  function _sim5RunDefaultConfigComparison({ seeds, trialsPerSeed } = {}) {
    seeds         = Array.isArray(seeds) && seeds.length > 0 ? seeds : [1001, 2001, 3001];
    trialsPerSeed = trialsPerSeed || 3;

    let _gDcc;
    try { _gDcc = eval('G'); } catch(e) { return null; }
    const realGStr = JSON.stringify(_gDcc);
    let gSnap;
    try { gSnap = _sim3MakeFreshStartSnap(_gDcc); } catch(e) { return null; }
    ...
    // 1シナリオ実行 → metrics（per-seedプロフィット配列付き）を返す
    const runScenarioDcc = (enable) => {
      Object.assign(_SIM5_ENABLE, enable);
      _sim5AdCurrentPolicy = null;
      _sim5AdPolicyState.lastPurchaseDayByStore = {};

      const seedProfits = [];
      let totalProfit = 0, totalRevenue = 0, totalEndCash = 0;
      let totalMinCash = 0, totalDeficitMonths = 0, totalBankruptcy = 0;
      let totalInvest = 0, totalTrain = 0, totalMenu = 0, totalAd = 0;
      let trialCount = 0;
      ...
      for (const seed of seeds) {
        ...
        const agg = _sim3AnalyzeAggregate(trials);
        const g = k => agg.stats[k]?.mean ?? 0;
        const seedProfit = g('totalNetChange365');
        seedProfits.push(seedProfit);
        totalProfit        += seedProfit;
        totalRevenue       += g('totalRevenue');
        totalEndCash       += g('endCash');
        totalMinCash       += g('minCash');
        totalDeficitMonths += g('deficitMonths');
        totalBankruptcy    += trials.filter(t => (t.minCash ?? 0) < 0).length / trials.length;
        totalInvest += _sim5Diag.attempted;
        totalTrain  += _sim5TrainDiag.attempted;
        totalMenu   += _sim5MenuDiag.attempted;
        totalAd     += _sim5AdDiag.attempted;
        trialCount++;
      }
      if (!trialCount) return null;
      const n = trialCount;
      return {
        profit:        totalProfit / n,
        revenue:       totalRevenue / n,
        endCash:       totalEndCash / n,
        minCash:       totalMinCash / n,
        deficitMonths: totalDeficitMonths / n,
        bankruptcyRate:totalBankruptcy / n,
        investCount:   totalInvest / n,
        trainCount:    totalTrain / n,
        menuCount:     totalMenu / n,
        adCount:       totalAd / n,
        daysNoAction:  null, // 将来拡張用
        _seedProfits:  seedProfits,
      };
    };

    try {
      const rawResults = [];
      for (let si = 0; si < _SIM5_DEFAULT_CONFIG_SCENARIOS.length; si++) {
        const sc = _SIM5_DEFAULT_CONFIG_SCENARIOS[si];
        console.log(`[DEFAULT CONFIG] seed 1/${seeds.length} scenario ${si+1}/${_SIM5_DEFAULT_CONFIG_SCENARIOS.length}: ${sc.label}`);
        const metrics = runScenarioDcc(sc.enable);
        if (!metrics) return null;
        rawResults.push({ key: sc.key, label: sc.label, enable: sc.enable, metrics });
      }
      console.log('[DEFAULT CONFIG] complete');

      // baseline = allOff
      const baseline = rawResults.find(r => r.key === 'allOff');
      if (!baseline) return null;
      const bm = baseline.metrics;

      // 各シナリオ: diffFromAllOff + 勝率統計
      const scenarios = rawResults.map(r => {
        const m = r.metrics;
        const profitDiffs = m._seedProfits.map(p => p - bm.profit);
        const stats   = _sim5CalcStats(profitDiffs);
        const wins    = profitDiffs.filter(v => v > 0).length;
        const winRate = seeds.length > 0 ? wins / seeds.length : 0;
        const diffFromAllOff = {
          profit:        m.profit        - bm.profit,
          revenue:       m.revenue       - bm.revenue,
          endCash:       m.endCash       - bm.endCash,
          minCash:       m.minCash       - bm.minCash,
          deficitMonths: m.deficitMonths - bm.deficitMonths,
          bankruptcyRate:m.bankruptcyRate - bm.bankruptcyRate,
        };
        return {
          key: r.key, label: r.label, enable: r.enable,
          metrics: m, diffFromAllOff,
          profitStats: stats, winRate,
        };
      });
      ...
      // コンソール表示
      {
        ...
        scenarios.forEach(s => {
          ...
          tableData[s.label] = {
            ...
            '最低現金':       fmt0(m.minCash),
            '赤字月数':       fmt0(m.deficitMonths),
            '倒産率':         pct(m.bankruptcyRate),
            '地域活動':       fmt0(m.investCount),
            '研修':           fmt0(m.trainCount),
            'メニュー追加':   fmt0(m.menuCount),
            '広告':           fmt0(m.adCount),
          };
        });
        console.group('📊 デフォルトAI構成 比較');
        console.table(tableData);
        ...
        console.table({
          '本番デフォルト vs 旧デフォルト': {
            ...
            '最低現金差分': sign(improvementFromOldDefault.minCash),
            '赤字月数差分': sign(improvementFromOldDefault.deficitMonths),
            '倒産率差分':   (improvementFromOldDefault.bankruptcyRate * 100).toFixed(1) + 'pp',
          },
        });
        console.groupEnd();
        ...
        console.table({
          '本番デフォルト vs メニューのみ': {
            ...
            '最低現金差分': sign(newDefaultVsMenuOnly.minCash),
            '赤字月数差分': sign(newDefaultVsMenuOnly.deficitMonths),
            '倒産率差分':   (newDefaultVsMenuOnly.bankruptcyRate * 100).toFixed(1) + 'pp',
          },
        });
        console.groupEnd();
      ...
      return {
        config: { seeds, trialsPerSeed, thresholds: _SIM5_DEFAULT_CONFIG_THRESHOLDS },
        scenarios,
  ...
  window._sim5RunDefaultConfigComparison = _sim5RunDefaultConfigComparison;

  // ── v0.5i: 広告ポリシー比較 ──────────────────────────────────────
  const _SIM5_AD_POLICY_SCENARIOS = [
    { key: 'adOff',    label: '広告なし',   enabled: false, cooldownDays: null },
    { key: 'adDaily',  label: '毎日判定',   enabled: true,  cooldownDays: 0    },
    { key: 'ad7Days',  label: '7日間隔',    enabled: true,  cooldownDays: 7    },
  ...
  function _sim5RunAdPolicyComparison({ seeds, trialsPerSeed, scenarios } = {}) {
    ...
    // 現在のGとフレッシュスナップ
    let _gApc;
    try { _gApc = eval('G'); } catch(e) { return null; }
    const realGStr = JSON.stringify(_gApc);
    let gSnap;
    try { gSnap = _sim3MakeFreshStartSnap(_gApc); } catch(e) { return null; }
    ...
    // 1ポリシー × 全seedで試行を実行し、集計メトリクスを返す
    const runPolicy = (pol) => {
      // 広告以外のAIはすべてOFF、広告のみポリシーに従う
      _SIM5_ENABLE.investRegion = false;
      _SIM5_ENABLE.trainStaff   = false;
      _SIM5_ENABLE.buyMenu      = false;
      _SIM5_ENABLE.buyAd        = pol.enabled;
      _sim5AdCurrentPolicy      = pol.enabled ? pol : null;

      // seed横断で集める値
      const profitDiffs_arr   = [];
      const revenueDiffs_arr  = [];
      const endCashDiffs_arr  = [];

      let totalProfit = 0, totalRevenue = 0, totalEndCash = 0;
      let totalMinCash = 0, totalDeficitMonths = 0, totalBankruptcy = 0;
      let totalAdAttempted = 0, totalAdPurchased = 0, totalCooldownBlocked = 0;
      let totalAdCostTotal = 0;
      let trialCount = 0;

      for (const seed of seeds) {
        _sim5AdDiag.reset();
        _sim5AdPolicyState.lastPurchaseDayByStore = {}; // seedごとにリセット
        ...
        totalProfit        += g('totalNetChange365');
        totalRevenue       += g('totalRevenue');
        totalEndCash       += g('endCash');
        totalMinCash       += g('minCash');
        totalDeficitMonths += g('deficitMonths');
        totalBankruptcy    += trials.filter(t => (t.minCash ?? 0) < 0).length / trials.length;
        totalAdAttempted      += _sim5AdDiag.attempted;
        totalAdPurchased      += _sim5AdDiag.adPurchased;
        totalCooldownBlocked  += _sim5AdDiag.cooldownBlocked;
        totalAdCostTotal      += _sim5AdDiag.adCostTotal;
        trialCount++;

        profitDiffs_arr.push(g('totalNetChange365'));
        revenueDiffs_arr.push(g('totalRevenue'));
        endCashDiffs_arr.push(g('endCash'));
      }
      ...
      return {
        ...
        deficitMonths:    totalDeficitMonths / n,
        bankruptcyRate:   totalBankruptcy / n,
        adCount:          totalAdAttempted / n,
        adPurchased:      totalAdPurchased / n,
        cooldownBlocked:  totalCooldownBlocked / n,
        adCostTotal:      totalAdCostTotal / n,
        _profitArr:       profitDiffs_arr,
        _revenueArr:      revenueDiffs_arr,
        _endCashArr:      endCashDiffs_arr,
      };
    };
    ...
    try {
      ...
      for (const pol of scenarios) {
        ...
        if (!metrics) return null;
        rawResults.push({ key: pol.key, label: pol.label, cooldownDays: pol.cooldownDays, enabled: pol.enabled, metrics });
      }

      // 基準: adOff
      const baseline = rawResults.find(r => r.key === 'adOff');
      if (!baseline) return null;
      const bm = baseline.metrics;

      // 各ポリシーに差分・1広告あたり指標・安定性統計を付与
      const policies = rawResults.map(r => {
        const m = r.metrics;
        const diffFromAdOff = {
          profit:        m.profit        - bm.profit,
          revenue:       m.revenue       - bm.revenue,
        ...
        const adCount    = m.adCount    || 0;
        const adCostTotal = m.adCostTotal || 0;
        const perActionMetrics = {
          profitDiffPerAd:      safe(diffFromAdOff.profit,  adCount),
          revenueDiffPerAd:     safe(diffFromAdOff.revenue, adCount),
          revenueReturnPerAdCost: safe(diffFromAdOff.revenue, adCostTotal),
          adCount,
          adCostTotal,
          cooldownBlocked: m.cooldownBlocked,
        };
        // 安定性統計（v0.5h の共通ヘルパーを再利用）
        const profitDiffArr = m._profitArr.map(p => p - bm.profit);
        const stability = _sim5CalcStats(profitDiffArr);
        const winRate   = profitDiffArr.filter(v => v > 0).length / (profitDiffArr.length || 1);
        const stabilityScore = stability.stddev > 0 ? stability.mean / stability.stddev : 0;

        return {
          key: r.key, label: r.label, cooldownDays: r.cooldownDays,
          metrics: m, diffFromAdOff, perActionMetrics,
          stability: { ...stability, winRate, stabilityScore },
        };
      });

      // ランキング（adOff除く・利益差分降順、同額なら最低現金高い順）
      const rankTargets = policies.filter(p => p.key !== 'adOff');
      rankTargets.sort((a, b) => {
        const pd = b.diffFromAdOff.profit - a.diffFromAdOff.profit;
        if (pd !== 0) return pd;
        return b.metrics.minCash - a.metrics.minCash;
      });
      const rankings = rankTargets.map((p, i) => ({
        rank:            i + 1,
        key:             p.key,
        label:           p.label,
        cooldownDays:    p.cooldownDays,
        profitDiff:      p.diffFromAdOff.profit,
        revenueDiff:     p.diffFromAdOff.revenue,
        stabilityScore:  p.stability.stabilityScore,
        winRate:         p.stability.winRate,
      }));
      ...
      // コンソール表示
      {
        ...
        policies.forEach(p => {
          const m = p.metrics;
          const d = p.diffFromAdOff;
          const pa = p.perActionMetrics;
          tableData[p.label] = {
            ...
            '最低現金':            fmt0(m.minCash),
            '赤字月数':            fmt0(m.deficitMonths),
            '広告回数':            fmt0(pa.adCount),
            '広告費':              fmt0(pa.adCostTotal),
            'クールダウン阻止':    fmt0(pa.cooldownBlocked),
            '1広告あたり利益差分': pa.adCount > 0 ? Math.round(pa.profitDiffPerAd).toLocaleString() : '-',
            '1広告あたり売上差分': pa.adCount > 0 ? Math.round(pa.revenueDiffPerAd).toLocaleString() : '-',
          };
        });
        console.group('📊 広告AI ポリシー比較');
        console.table(tableData);
        console.group('🏆 広告ポリシーランキング（利益差分降順）');
        rankings.forEach(r =>
          console.log(`${r.rank}. ${r.label}: 利益差分 ${sign(r.profitDiff)} / 安定指数 ${r.stabilityScore.toFixed(2)} / 勝率 ${pct(r.winRate)}`)
        );
        console.groupEnd();
        console.groupEnd();
      }

      return { policies, rankings, seeds, trialsPerSeed };
  ...
  function _sim6BuildAdoptionConsistencyReport({ adoptionStatus, evaluations } = {}) {
    ...
    const items = keys.map(key => {
      ...
      if (!ev) {
        notes = 'データ不足: 評価結果なし';
      } else if (cfg.status === 'candidate') {
        // candidate ステータスは評価完了済みだが正式採用未反映のため consistent=null
        consistent = null;
        notes = '評価完了・正式採用未反映';
  ...
  function _sim3AnalyzeRunTrial(seed, gSnap) {
    ...
    try {
      ...
      for (let d = 0; d < GAME_DAYS_PER_YEAR; d++) {
        ...
        _sim5AdCurrentDay = d; // v0.5i: クールダウン判定用に現在日を同期
        ...
        if (_SIM5_ENABLE.buyRenov)     _sim7TryBuyRenov(); // v0.7: 改装AI（デフォルトOFF）
...
<div style="background:#0d1f2d;border:1px solid #1e3a5a;border-radius:6px;padding:10px 14px;margin-bottom:8px">
  ...
  function _sim3BalanceSimulator(result, params) {
    ...
    // ── 商品解放数（解放条件・評判推移・未達差分が不明のため予測不可） ──
    // 実際の解放条件値・日別評判推移がないと到達可能性を計算できない。
    // 推測はしない。p.unlockCondMult が変わっても null のまま維持。
    const simProducts = avgProducts;
    const unlockPredictable = false; // 将来データが取得できれば true に変更
    ...
    // ── 改善効果ランキング（変更ありパラメータのみ） ──
    const rankings = [];
    if (p.revenueMult !== 1.0 && deltaRevenue != null)
      rankings.push({ item:`店舗売上 ${_sim3BalPct(p.revenueMult)}`, impact: deltaRevenue, known:true });
    if (p.expenseMult !== 1.0 && deltaExpense != null)
      rankings.push({ item:`経費合計 ${_sim3BalPct(p.expenseMult)}`, impact:-deltaExpense, known:true,
    ...
    if (p.apCostMult !== 1.0 || p.apRecoveryMult !== 1.0)
      rankings.push({ item:`AP（消費${_sim3BalPct(p.apCostMult)} / 回復${_sim3BalPct(p.apRecoveryMult)}）`, impact:null, known:false, note:'AP変化の金銭換算は情報不足。難易度への影響のみ反映。' });
    if (p.unlockCondMult !== 1.0)
      rankings.push({ item:`商品解放条件 ${_sim3BalPct(p.unlockCondMult)}`, impact:null, known:false, note:'予測不可。商品解放条件値・日別評判推移の計測が必要。' });
    rankings.sort((a,b) => {
    ...
    if (p.unlockCondMult !== 1.0)
      lines.push('【商品解放予測不可】実際の解放条件値・日別評判推移データがないため、解放数変化を推測しません。');
  ...
  function _sim3BalanceSimulatorHtml(sim, origBr) {
    ...
    const rows = [
      ...
      ['商品数', `${fmt0(b.products)}個`,
        sim.params.unlockCondMult !== 1.0 ? '予測不可' : `${fmt0(s2.products)}個`,
        sim.params.unlockCondMult !== 1.0 ? '' : arrow(b.products, s2.products), ''],
    ...
    // 改善効果ランキング（変更ありパラメータのみ。全0%時は空）
    const hasRankings = sim.rankings.length > 0;
    const rankRows = hasRankings ? sim.rankings.map((r, i) => {
...
<div style="background:#0a0a18;border:1px solid #1e3a5a;border-radius:6px;padding:8px 12px;margin-bottom:8px">
  ...
  ${hasRankings
    ...
    : `<div style="font-size:10px;color:#555;padding:6px 0">変更するパラメータを選択すると改善効果を表示します</div>`
  }
</div>
...
<div style="background:#080808;border:1px solid #222;border-radius:6px;padding:10px 14px;margin-bottom:8px">
  ...
  window._sim3BalUpdate = function() {
    ...
    if (window._QA_BALANCE_DEBUG) {
      console.log('[BalanceSim] params', {
        revenueMult:    params.revenueMult,    revenueRate: params.revenueMult - 1,
        expenseMult:    params.expenseMult,    expenseRate: params.expenseMult - 1,
        apCostMult:     params.apCostMult,     apRecoveryMult: params.apRecoveryMult,
        unlockCondMult: params.unlockCondMult, compEventFreq:  params.compEventFreq,
        caseRewardMult: params.caseRewardMult, adEffectMult:   params.adEffectMult,
      });
      if (sim) console.log('[BalanceSim] result', {
        netChange365:  sim.sim?.netChange365,
        storeProfit12: sim.sim?.storeProfit12,
        score:         sim.sim?.score,
        difficulty:    sim.sim?.difficulty,
        delta_net:     sim.delta?.netChange365,
      });
    }
  ...
  // ─── Phase 3B-2 バリデーション関数 ───
  window._qa3b2ValidateSimulator = function() {
    const r = window._qa3aAnalyzeResult;
    if (!r || !r.stats) {
      console.warn('[qa3b2] 先に _qa3aAnalyzeRun() を実行してください');
      return null;
    }
    const s = r.stats;
    const g = k => s[k]?.mean ?? null;
    ...
    // A. 店舗売上 +10%
    const simA = _sim3BalanceSimulator(r, { revenueMult: 1.1 });
    const expA = avgRevenue != null ? Math.round(avgRevenue * 0.1) : null;
    const actA = simA?.delta?.netChange365 != null ? Math.round(simA.delta.netChange365) : null;
    const passA = expA != null && actA != null && Math.abs(actA - expA) <= 1;

    // B. 経費合計 -10%
    const simB = _sim3BalanceSimulator(r, { expenseMult: 0.9 });
    // expenseMult=0.9 → deltaExpense = avgTotalExpense*(0.9-1) = -0.1*avgTotalExpense
    // impact = -deltaExpense = +0.1*avgTotalExpense
    const expB = avgTotalExpense != null ? Math.round(avgTotalExpense * 0.1) : null;
    const actB = simB?.delta?.netChange365 != null ? Math.round(simB.delta.netChange365) : null;
    const passB = expB != null && actB != null && Math.abs(actB - expB) <= 1;

    // C. 複合（A+B）
    const simC = _sim3BalanceSimulator(r, { revenueMult: 1.1, expenseMult: 0.9 });
    const expC = (expA != null && expB != null) ? expA + expB : null;
    const actC = simC?.delta?.netChange365 != null ? Math.round(simC.delta.netChange365) : null;
    const passC = expC != null && actC != null && Math.abs(actC - expC) <= 1;

    const allPass = passA && passB && passC;

    const out = {
      baseData: {
        avgRevenue: avgRevenue != null ? Math.round(avgRevenue) : null,
        avgTotalExpense: avgTotalExpense != null ? Math.round(avgTotalExpense) : null,
        avgNetChange365: avgNetChange365 != null ? Math.round(avgNetChange365) : null,
      },
      revenuePlus10:  { expected: expA, actual: actA, pass: passA },
      expenseMinus10: { expected: expB, actual: actB, pass: passB },
      combined:       { expected: expC, actual: actC, pass: passC },
      allPass,
    };

    console.log('[qa3b2] バランスシミュレーター検証結果');
    console.table({
      'A. 店舗売上+10%':  { 期待値: expA, 実際: actA, 差: actA != null && expA != null ? actA-expA : null, 判定: passA?'✅ PASS':'❌ FAIL' },
      'B. 経費合計-10%':  { 期待値: expB, 実際: actB, 差: actB != null && expB != null ? actB-expB : null, 判定: passB?'✅ PASS':'❌ FAIL' },
      'C. A+B複合':       { 期待値: expC, 実際: actC, 差: actC != null && expC != null ? actC-expC : null, 判定: passC?'✅ PASS':'❌ FAIL' },
    });
    console.log(`[qa3b2] 総合: ${allPass ? '✅ allPass = true' : '❌ allPass = false'}`);
    window._qa3b2ValidateResult = out;
    return out;
  };
...
`;
  ...
  window._qa3ValidateAll = async function(opts) {
    ...
    // 14-6: 購入可能広告なしならスキップ
    // AD_OPTIONS は const のため再代入不可 → _sim5TryBuyAd の _adOpts 引数で空配列を渡す
    {
      const snap = Object.assign({}, _sim5AdDiag);
      const origG = eval('G');
    ...
    // v0.5k: reset()→finally復元パターンを廃止しデルタパターンに変更（snap=0問題の回避）
    {
      ...
      const origG = eval('G');
      const mockAds = [
        { name: '高額広告', cost: 100_000, cust: 20 },
        { name: '最安広告', cost: 30_000,  cust: 5  },
      ];
      let calledArgs = null;
      const mockBuyAdFn = (si, name, cost, cust) => {
        calledArgs = { si, name, cost, cust };
    ...
    // ── Section 16: v0.5g AI行動別 限界効果分析 検証 ──────────────
    console.group('Section 16: v0.5g AI行動別 限界効果分析（Marginal Impact）検証');

    // 16-1: _sim5RunMarginalImpactAnalysis 存在確認
    typeof _sim5RunMarginalImpactAnalysis === 'function'
      ? pass('16-1 _sim5RunMarginalImpactAnalysis 存在', '関数として定義済み')
      : fail('16-1 _sim5RunMarginalImpactAnalysis 存在', `typeof=${typeof _sim5RunMarginalImpactAnalysis}`);

    // 16-2: _SIM5_IMPACT_SCENARIOS 定義確認（AI数+2シナリオ）
    {
      const expected16_2 = _SIM5_IMPACT_SCENARIOS.length;
      Array.isArray(_SIM5_IMPACT_SCENARIOS) && expected16_2 > 0
        ? pass(`16-2 _SIM5_IMPACT_SCENARIOS ${expected16_2}シナリオ`, `keys=${_SIM5_IMPACT_SCENARIOS.map(s=>s.key).join(',')}`)
        : fail(`16-2 _SIM5_IMPACT_SCENARIOS シナリオ`, `length=${_SIM5_IMPACT_SCENARIOS?.length}`);
    }

    // 16-3〜16-12: 実行して結果を検証
    const marginalResult = typeof _sim5RunMarginalImpactAnalysis === 'function'
      ? _sim5RunMarginalImpactAnalysis(numTrials, seed) : null;

    // BusinessReport に aiMarginalImpact を付与
    if (rNew && rNew.businessReport) rNew.businessReport.aiMarginalImpact = marginalResult;

    // 16-3: 実行成功（nullでない）
    marginalResult !== null && marginalResult !== undefined
      ? pass('16-3 実行成功', `シナリオ数=${marginalResult?.scenarios?.length}`)
      : fail('16-3 実行成功', 'marginalResult が null');

    // 16-4: baselineKey が 'allOff'
    marginalResult?.baselineKey === 'allOff'
      ? pass('16-4 baselineKey', `baselineKey=${marginalResult.baselineKey}`)
      : fail('16-4 baselineKey', `baselineKey=${marginalResult?.baselineKey}`);

    // 16-5: scenarios 配列が _SIM5_IMPACT_SCENARIOS と同数
    {
      const expected16_5 = _SIM5_IMPACT_SCENARIOS.length;
      (Array.isArray(marginalResult?.scenarios) && marginalResult.scenarios.length === expected16_5)
        ? pass(`16-5 scenarios ${expected16_5}件`, `keys=${marginalResult.scenarios.map(s=>s.key).join(',')}`)
        : fail(`16-5 scenarios ${expected16_5}件`, `length=${marginalResult?.scenarios?.length}`);
    }

    // 16-6: allOff の diffFromBaseline が全0
    {
      const allOff = marginalResult?.scenarios?.find(s => s.key === 'allOff');
      const d = allOff?.diffFromBaseline;
      const isZero = d && Object.values(d).every(v => v === 0);
      isZero
        ? pass('16-6 allOff diff全0', `profit_diff=${d?.profit}`)
        : fail('16-6 allOff diff全0', `diff=${JSON.stringify(d)}`);
    }

    // 16-7: allOn の profit >= allOff の profit（全AI有りは無しより利益が高いはず）
    {
      const allOff_ = marginalResult?.scenarios?.find(s => s.key === 'allOff');
      const allOn_  = marginalResult?.scenarios?.find(s => s.key === 'allOn');
      const p_off = allOff_?.metrics?.profit;
      const p_on  = allOn_?.metrics?.profit;
      (typeof p_off === 'number' && typeof p_on === 'number')
        ? pass('16-7 利益数値取得', `allOff=${Math.round(p_off).toLocaleString()} allOn=${Math.round(p_on).toLocaleString()} diff=${Math.round(p_on-p_off).toLocaleString()}`)
        : fail('16-7 利益数値取得', `p_off=${p_off} p_on=${p_on}`);
    }

    // 16-8: 単独AIシナリオ4件に costEfficiency が存在
    {
      const soloKeys = ['investRegionOnly','trainStaffOnly','buyMenuOnly','buyAdOnly'];
      const solos = marginalResult?.scenarios?.filter(s => soloKeys.includes(s.key)) || [];
      const allHaveCE = solos.length === 4 && solos.every(s => s.costEfficiency !== null && s.costEfficiency !== undefined);
      allHaveCE
        ? pass('16-8 単独AI costEfficiency 存在', `${solos.map(s=>`${s.key}:${Math.round(s.costEfficiency?.incrementalProfitPerAction||0)}`).join(',')}`)
        : fail('16-8 単独AI costEfficiency 存在', `solos=${solos.length} missing=${solos.filter(s=>!s.costEfficiency).map(s=>s.key).join(',')}`);
    }

    // 16-9: allOn に costEfficiency が null
    {
      const allOn_ = marginalResult?.scenarios?.find(s => s.key === 'allOn');
      allOn_?.costEfficiency === null
        ? pass('16-9 allOn costEfficiency=null', 'OK')
        : fail('16-9 allOn costEfficiency=null', `costEfficiency=${JSON.stringify(allOn_?.costEfficiency)}`);
    }

    // 16-10: rankings 配列が単独AIシナリオ数と一致（allOff/allOn除く）
    {
      const expected16_10 = _SIM5_IMPACT_SCENARIOS.length - 2;
      (Array.isArray(marginalResult?.rankings) && marginalResult.rankings.length === expected16_10)
        ? pass(`16-10 rankings ${expected16_10}件`, `top1=${marginalResult.rankings[0]?.label}`)
        : fail(`16-10 rankings ${expected16_10}件`, `length=${marginalResult?.rankings?.length}`);
    }

    // 16-11: rankings が利益差分降順
    {
      const ranks = marginalResult?.rankings || [];
      let ordered = true;
      for (let i = 1; i < ranks.length; i++) {
        if (ranks[i].profitDiff > ranks[i-1].profitDiff) { ordered = false; break; }
      }
      const expected16_11 = _SIM5_IMPACT_SCENARIOS.length - 2;
      ordered && ranks.length === expected16_11
        ? pass('16-11 rankings 利益差分降順', ranks.map(r=>`${r.label}:${Math.round(r.profitDiff)}`).join(' > '))
        : fail('16-11 rankings 利益差分降順', `ordered=${ordered} ranks=${JSON.stringify(ranks.map(r=>r.profitDiff))}`);
    }

    // 16-12: G の復元確認（実行後も G が正常）
    {
      let gAfterMarginal;
      try { gAfterMarginal = eval('G'); } catch(e) { gAfterMarginal = null; }
    ...
    // 17-3: シナリオ存在（_SIM5_IMPACT_SCENARIOS と同数）
    {
      const expected17_3 = _SIM5_IMPACT_SCENARIOS.length;
      (Array.isArray(stabilityResult?.scenarios) && stabilityResult.scenarios.length === expected17_3)
        ? pass(`17-3 ${expected17_3}シナリオ存在`, stabilityResult.scenarios.map(s=>s.key).join(','))
        : fail(`17-3 ${expected17_3}シナリオ存在`, `length=${stabilityResult?.scenarios?.length}`);
    }
    ...
    // 17-10: 順位件数一致（単独AIシナリオ数 = allOff/allOn除く）
    {
      const expected17_10 = _SIM5_IMPACT_SCENARIOS.length - 2;
      (Array.isArray(stabilityResult?.stabilityRanking) && stabilityResult.stabilityRanking.length === expected17_10)
        ? pass(`17-10 順位件数一致 ${expected17_10}件`, `top1=${stabilityResult.stabilityRanking[0]?.label} score=${stabilityResult.stabilityRanking[0]?.stabilityScore?.toFixed(2)}`)
        : fail(`17-10 順位件数一致 ${expected17_10}件`, `length=${stabilityResult?.stabilityRanking?.length}`);
    }
    ...
    // 17-12: console.table用データ生成成功（シナリオ全件）
    {
      const expected17_12 = _SIM5_IMPACT_SCENARIOS.length;
      const tableOk = stabilityResult?.scenarios?.length === expected17_12
      ...
      tableOk
        ...
        : fail('17-12 console.table用データ生成成功', `scenarios=${stabilityResult?.scenarios?.length} expected=${expected17_12}`);
    }

    console.groupEnd();

    // ── Section 18: v0.5i 広告ポリシー比較 検証 ──────────────────
    console.group('Section 18: v0.5i 広告AIポリシー比較（Ad Policy Comparison）検証');

    // Section 18 は seeds を少数に絞って実行（3seed × 3trials）
    const adPolSeeds        = [1001, 2001, 3001];
    const adPolTrialsPerSeed = 3;

    // 18-1: _sim5RunAdPolicyComparison 存在確認
    typeof _sim5RunAdPolicyComparison === 'function'
      ? pass('18-1 _sim5RunAdPolicyComparison 存在', '関数として定義済み')
      : fail('18-1 _sim5RunAdPolicyComparison 存在', `typeof=${typeof _sim5RunAdPolicyComparison}`);

    // 18-2: 5ポリシーが定義されている
    Array.isArray(_SIM5_AD_POLICY_SCENARIOS) && _SIM5_AD_POLICY_SCENARIOS.length === 5
      ? pass('18-2 5ポリシー定義', _SIM5_AD_POLICY_SCENARIOS.map(p=>p.key).join(','))
      : fail('18-2 5ポリシー定義', `length=${_SIM5_AD_POLICY_SCENARIOS?.length}`);

    // ポリシー比較を実行（18-3〜18-14 の検証用）
    const adPolResult = typeof _sim5RunAdPolicyComparison === 'function'
      ? _sim5RunAdPolicyComparison({ seeds: adPolSeeds, trialsPerSeed: adPolTrialsPerSeed }) : null;

    // BusinessReport に adPolicyComparison を付与
    if (rNew && rNew.businessReport) rNew.businessReport.adPolicyComparison = adPolResult
      ? { policies: adPolResult.policies, rankings: adPolResult.rankings } : null;

    const _adPol = (key) => adPolResult?.policies?.find(p => p.key === key);

    // 18-3: 広告なしで広告回数が0
    {
      const adOff = _adPol('adOff');
      const cnt   = adOff?.perActionMetrics?.adCount ?? -1;
      cnt === 0
        ? pass('18-3 adOff 広告回数0', `adCount=${cnt}`)
        : fail('18-3 adOff 広告回数0', `adCount=${cnt}`);
    }

    // 18-4: 毎日判定でcooldownBlocked=0
    {
      const adDaily = _adPol('adDaily');
      const cb = adDaily?.perActionMetrics?.cooldownBlocked ?? -1;
      cb === 0
        ? pass('18-4 adDaily cooldownBlocked=0', `cooldownBlocked=${cb}`)
        : fail('18-4 adDaily cooldownBlocked=0', `cooldownBlocked=${cb}`);
    }

    // 18-5〜18-7: クールダウン設定ポリシーの広告回数が毎日判定より少ない（単調減少）
    {
      const daily  = _adPol('adDaily')?.perActionMetrics?.adCount ?? 0;
      const d7     = _adPol('ad7Days')?.perActionMetrics?.adCount ?? 0;
      const d14    = _adPol('ad14Days')?.perActionMetrics?.adCount ?? 0;
      const d30    = _adPol('ad30Days')?.perActionMetrics?.adCount ?? 0;
      daily >= d7
        ? pass('18-5 7日間隔で広告回数 ≤ 毎日', `daily=${daily.toFixed(1)} 7d=${d7.toFixed(1)}`)
        : fail('18-5 7日間隔で広告回数 ≤ 毎日', `daily=${daily.toFixed(1)} 7d=${d7.toFixed(1)}`);
      d7 >= d14
        ? pass('18-6 14日間隔で広告回数 ≤ 7日間隔', `7d=${d7.toFixed(1)} 14d=${d14.toFixed(1)}`)
        : fail('18-6 14日間隔で広告回数 ≤ 7日間隔', `7d=${d7.toFixed(1)} 14d=${d14.toFixed(1)}`);
      d14 >= d30
        ? pass('18-7 30日間隔で広告回数 ≤ 14日間隔', `14d=${d14.toFixed(1)} 30d=${d30.toFixed(1)}`)
        : fail('18-7 30日間隔で広告回数 ≤ 14日間隔', `14d=${d14.toFixed(1)} 30d=${d30.toFixed(1)}`);
    }

    // 18-8: cooldownBlocked 増加の単体検証
    {
      let cbBefore18_8, cbDelta18_8, res18_8;
      const snap18_8 = Object.assign({}, _sim5AdDiag);
      const origG18_8 = eval('G');
      const origPol18_8 = _sim5AdCurrentPolicy;
      try {
        // 7日クールダウン・現在日=3・最後の購入日=0 → ブロックされるはず
        eval('G = { stores: [{ name:"本店", isOpen:true, customers:20 }], staff:[], money:1_000_000, ap:100 }');
        _sim5AdCurrentPolicy = { cooldownDays: 7, enabled: true };
        _sim5AdPolicyState.lastPurchaseDayByStore = { 0: 0 };
        _sim5AdCurrentDay = 3; // 3-0=3 < 7 → クールダウン
        cbBefore18_8 = _sim5AdDiag.cooldownBlocked;
        res18_8 = _sim5TryBuyAd();
        cbDelta18_8 = _sim5AdDiag.cooldownBlocked - cbBefore18_8;
      } finally {
        eval('G = origG18_8');
        _sim5AdCurrentPolicy = origPol18_8;
        _sim5AdPolicyState.lastPurchaseDayByStore = {};
        _sim5AdCurrentDay = 0;
        for (const [k, v] of Object.entries(snap18_8)) { if (typeof v === 'number') _sim5AdDiag[k] = v; }
      }
      (res18_8 === false && cbDelta18_8 === 1)
        ? pass('18-8 cooldownBlocked増加', `res=${res18_8} cbDelta=${cbDelta18_8}`)
        : fail('18-8 cooldownBlocked増加', `res=${res18_8} cbDelta=${cbDelta18_8}`);
    }

    // 18-9: 全ポリシーの数値がFinite
    {
      const ok = adPolResult?.policies?.every(p =>
        isFinite(p.metrics.profit) && isFinite(p.metrics.revenue) &&
        isFinite(p.diffFromAdOff.profit) && isFinite(p.perActionMetrics.adCount)
      );
      ok
        ? pass('18-9 全ポリシー数値Finite', `policies=${adPolResult?.policies?.length}`)
        : fail('18-9 全ポリシー数値Finite', 'Finite check failed');
    }

    // 18-10: adOff との差分が再計算値と一致（adDaily で検証）
    {
      const adOff   = _adPol('adOff');
      const adDaily = _adPol('adDaily');
      if (adOff && adDaily) {
        const expected = adDaily.metrics.profit - adOff.metrics.profit;
        const actual   = adDaily.diffFromAdOff.profit;
        Math.abs(expected - actual) < 1
          ? pass('18-10 差分再計算一致', `expected=${Math.round(expected)} actual=${Math.round(actual)}`)
          : fail('18-10 差分再計算一致', `expected=${Math.round(expected)} actual=${Math.round(actual)}`);
      } else {
        fail('18-10 差分再計算一致', 'adOff or adDaily が null');
      }
    }

    // 18-11: 1広告あたり指標が再計算値と一致
    {
      const adDaily = _adPol('adDaily');
      if (adDaily && adDaily.perActionMetrics.adCount > 0) {
        const expected = adDaily.diffFromAdOff.profit / adDaily.perActionMetrics.adCount;
        const actual   = adDaily.perActionMetrics.profitDiffPerAd;
        Math.abs(expected - actual) < 1
          ? pass('18-11 1広告あたり指標一致', `expected=${Math.round(expected)} actual=${Math.round(actual)}`)
          : fail('18-11 1広告あたり指標一致', `expected=${Math.round(expected)} actual=${Math.round(actual)}`);
      } else {
        pass('18-11 1広告あたり指標一致', 'adCount=0のためスキップ（広告購入なし）');
      }
    }

    // 18-12: ランキングが利益差分降順
    {
      const ranks = adPolResult?.rankings || [];
      let ordered = true;
      for (let i = 1; i < ranks.length; i++) {
        if (ranks[i].profitDiff > ranks[i-1].profitDiff) { ordered = false; break; }
      }
      ordered && ranks.length === 4
        ? pass('18-12 ランキング降順', ranks.map(r=>`${r.label}:${Math.round(r.profitDiff)}`).join(' ≥ '))
        : fail('18-12 ランキング降順', `ordered=${ordered} length=${ranks.length}`);
    }

    // 18-13: BusinessReport.adPolicyComparison 存在
    (rNew?.businessReport?.adPolicyComparison?.policies?.length === 5)
      ? pass('18-13 BusinessReport.adPolicyComparison存在', `policies=${rNew.businessReport.adPolicyComparison.policies.length}`)
      : fail('18-13 BusinessReport.adPolicyComparison存在', `${JSON.stringify(rNew?.businessReport?.adPolicyComparison)?.slice(0,60)}`);

    // 18-14: 既存AI比較レポート整合性（cooldownBlocked を含む otherBlockedKeys 拡張確認）
    {
      const adDef = _SIM5_ACTION_DEFS?.find(d => d.key === 'buyAd');
      // cooldownBlocked は広告固有阻害理由として _sim5AdDiag に存在すること確認
      const hasCB = typeof _sim5AdDiag.cooldownBlocked === 'number';
      hasCB
    ...
    // ── Section 21: v0.5l デフォルトAI構成 比較検証 ─────────────────
    console.group('Section 21: v0.5l デフォルトAI構成 比較検証');

    let defCmpResult21 = null;

    // 21-1: _sim5RunDefaultConfigComparison が存在
    typeof _sim5RunDefaultConfigComparison === 'function'
      ? pass('21-1 _sim5RunDefaultConfigComparison 存在', '関数として定義済み')
      : fail('21-1 _sim5RunDefaultConfigComparison 存在', `typeof=${typeof _sim5RunDefaultConfigComparison}`);

    // 21-2: 4シナリオが定義されている
    (Array.isArray(_SIM5_DEFAULT_CONFIG_SCENARIOS) && _SIM5_DEFAULT_CONFIG_SCENARIOS.length === 4)
    ...
    {
      const enableSnap21 = Object.assign({}, _SIM5_ENABLE);
      const diagSnap21   = [_sim5Diag, _sim5TrainDiag, _sim5MenuDiag, _sim5AdDiag, _sim7RenovDiag].map(d => Object.assign({}, d));
      try {
        defCmpResult21 = typeof _sim5RunDefaultConfigComparison === 'function'
          ? _sim5RunDefaultConfigComparison({ seeds: [1001, 2001, 3001], trialsPerSeed: 3 })
          : null;
      } catch(e21) {
        defCmpResult21 = null;
      }

      // 復元確認用: 関数のfinally後の状態
      const enableAfter21 = Object.assign({}, _SIM5_ENABLE);
      ...
      {
        const m = allOff21?.metrics;
        const ok21_5 = m && m.investCount === 0 && m.trainCount === 0 && m.menuCount === 0 && m.adCount === 0;
        ok21_5
          ? pass('21-5 allOff AI行動回数=0', `invest=${m.investCount} train=${m.trainCount} menu=${m.menuCount} ad=${m.adCount}`)
          : fail('21-5 allOff AI行動回数=0', `m=${JSON.stringify({invest:m?.investCount,train:m?.trainCount,menu:m?.menuCount,ad:m?.adCount})}`);
      }

      // 21-6: oldDefaultの広告回数が1以上
      {
        const adCount21_6 = oldDef21?.metrics?.adCount;
        (adCount21_6 != null && adCount21_6 >= 1)
          ? pass('21-6 oldDefault 広告回数≥1', `adCount=${adCount21_6}`)
      ...
      {
        const keys8 = ['profit','revenue','endCash','minCash','deficitMonths','bankruptcyRate'];
        const allFinite = defCmpResult21?.scenarios?.every(s => keys8.every(k => isFinite(s.metrics[k])));
        allFinite
          ? pass('21-8 全シナリオ統計値 Finite', `scenarios=${defCmpResult21.scenarios.length}`)
          : fail('21-8 全シナリオ統計値 Finite', `allFinite=${allFinite}`);
      }
      ...
      // 21-9: allOffとの差分が再計算値と一致
      {
        ...
        match9
          ...
          : fail('21-9 diffFromAllOff 再計算一致', `diff=${diff9} recomputed=${recomputed9}`);
      }
      ...
      // 21-10: improvementFromOldDefault が再計算値と一致
      {
        ...
        match10
          ...
          : fail('21-10 improvementFromOldDefault 再計算一致', `imp=${imp10} recomp=${recomputed10}`);
      }
      ...
      // 21-11: newDefaultVsMenuOnly が再計算値と一致
      {
        ...
        match11
          ...
          : fail('21-11 newDefaultVsMenuOnly 再計算一致', `vs=${vs11} recomp=${recomputed11}`);
      }

      // 21-12: adoptionEligible が全判定条件のANDと一致
      {
        const ad12  = defCmpResult21?.adoptionDecision;
        const reand12 = ad12 && Object.values(ad12.adoptionChecks).every(v => v);
        const match12 = ad12 && ad12.adoptionEligible === reand12;
        match12
          ? pass('21-12 adoptionEligible AND一致', `eligible=${ad12?.adoptionEligible} checks=${JSON.stringify(ad12?.adoptionChecks)}`)
          : fail('21-12 adoptionEligible AND一致', `eligible=${ad12?.adoptionEligible} recomputed=${reand12}`);
      }

      // 21-13: BusinessReport.defaultConfigComparison が存在
      (rNew?.businessReport?.defaultConfigComparison && Array.isArray(rNew.businessReport.defaultConfigComparison.scenarios))
    ...
    let aiAdoptionEvaluation23 = null;
    {
      ...
      const evalBuyRenov = makeEvalFromMarginal('buyRenov',     '店舗改装',       'buyRenovOnly');
      ...
      // コンソール表示
      {
        ...
        const tableData23 = {};
        const order = ['investRegion','trainStaff','buyMenu','buyAd','buyRenov'];
        order.forEach(k => {
    ...
    // ── Section 24: v0.7 buyRenov候補AI 検証 ─────────────────────
    console.group('Section 24: v0.7 buyRenov候補AI 検証');
    {
      // 24-1: _sim7TryBuyRenov 存在確認
      typeof _sim7TryBuyRenov === 'function'
        ? pass('24-1 _sim7TryBuyRenov 存在', '関数として定義済み')
        : fail('24-1 _sim7TryBuyRenov 存在', `typeof=${typeof _sim7TryBuyRenov}`);

      // -- ヘルパー: G・診断カウンター・_SIM5_ENABLEのバックアップ/リストア --
      const _24snapEnable = () => Object.assign({}, _SIM5_ENABLE);
      const _24restoreEnable = (snap) => { Object.assign(_SIM5_ENABLE, snap); };
      const _24snapDiag = () => ({
        called: _sim7RenovDiag.called, noStore: _sim7RenovDiag.noStore,
        noOpenStore: _sim7RenovDiag.noOpenStore, noAvailableRenov: _sim7RenovDiag.noAvailableRenov,
        insufficientCash: _sim7RenovDiag.insufficientCash, insufficientAP: _sim7RenovDiag.insufficientAP,
        attempted: _sim7RenovDiag.attempted, succeeded: _sim7RenovDiag.succeeded,
        renovPurchased: _sim7RenovDiag.renovPurchased, allPurchased: _sim7RenovDiag.allPurchased,
      });
      const _24restoreDiag = (snap) => { Object.keys(snap).forEach(k => { _sim7RenovDiag[k] = snap[k]; }); };

      let _24gStr = null;
      try { _24gStr = JSON.stringify(eval('G')); } catch(e) {}
      const _24restoreG = () => { if (_24gStr) { try { eval('G = JSON.parse(_24gStr)'); } catch(e) {} } };

      // 24-2: ストアなし → noStore カウント増加
      {
        const enSnap = _24snapEnable();
        const dSnap  = _24snapDiag();
        let delta = null;
        try {
          let _g24tmp;
          try { _g24tmp = eval('G'); } catch(e) {}
          const origStores = _g24tmp?.stores;
          try { eval('G.stores = []'); } catch(e) {}
          const diagBefore = _24snapDiag();
          try { _sim7TryBuyRenov(); } catch(e) {}
          const diagAfter = _24snapDiag();
          delta = diagAfter.noStore - diagBefore.noStore;
        } finally {
          _24restoreG();
          _24restoreDiag(dSnap);
          _24restoreEnable(enSnap);
        }
        delta === 1
          ? pass('24-2 noStore スキップ', `noStoreデルタ=${delta}`)
          : fail('24-2 noStore スキップ', `noStoreデルタ=${delta}（期待値=1）`);
      }

      // 24-3: 開店なし → noOpenStore カウント増加
      {
        const enSnap = _24snapEnable();
        const dSnap  = _24snapDiag();
        let delta = null;
        try {
          let _g24tmp;
          try { _g24tmp = eval('G'); } catch(e) {}
          if (_g24tmp?.stores?.length > 0) {
            const origIsOpen = _g24tmp.stores.map(s => s.isOpen);
            try { eval('G.stores.forEach(s => { s.isOpen = false; })'); } catch(e) {}
            const diagBefore = _24snapDiag();
            try { _sim7TryBuyRenov(); } catch(e) {}
            const diagAfter = _24snapDiag();
            delta = diagAfter.noOpenStore - diagBefore.noOpenStore;
          } else {
            delta = 0; // ストアがない場合はテスト不能
          }
        } finally {
          _24restoreG();
          _24restoreDiag(dSnap);
        ...
        delta === 1
          ? pass('24-3 noOpenStore スキップ', `noOpenStoreデルタ=${delta}`)
          : fail('24-3 noOpenStore スキップ', `noOpenStoreデルタ=${delta}（期待値=1）`);
      }

      // 24-4: 全改装購入済み → allPurchased カウント増加
      {
        const enSnap = _24snapEnable();
        const dSnap  = _24snapDiag();
        let delta = null;
        try {
          // renovOptionsOverride でテスト用オプションを注入（RENOV_OPTIONSに依存しない）
          const testRenov4 = [{ name: '__test_renov4__', cost: 1 }];
          // オープンストアを確保（なければstores[0]を強制オープン）
          let _g24tmp; try { _g24tmp = eval('G'); } catch(e) {}
          let si24 = _g24tmp?.stores?.findIndex(s => s.isOpen) ?? -1;
          if (si24 < 0 && (_g24tmp?.stores?.length ?? 0) > 0) {
            try { eval('G.stores[0].isOpen = true'); } catch(e) {}
            si24 = 0;
          }
          if (si24 >= 0) {
            // テスト用オプション名をupgradesに設定して allPurchased を発火させる
            try { eval(`G.stores[${si24}].upgrades = ['__test_renov4__']`); } catch(e) {}
            const diagBefore = _24snapDiag();
            try { _sim7TryBuyRenov(testRenov4); } catch(e) {}
            const diagAfter = _24snapDiag();
            delta = diagAfter.allPurchased - diagBefore.allPurchased;
          } else {
            delta = 0;
          }
        } finally {
        ...
        delta === 1
          ? pass('24-4 全改装済み allPurchased スキップ', `allPurchasedデルタ=${delta}`)
          : fail('24-4 全改装済み allPurchased スキップ', `allPurchasedデルタ=${delta}（期待値=1）`);
      }

      // 24-5: 資金不足 → insufficientCash カウント増加
      {
        const enSnap = _24snapEnable();
        const dSnap  = _24snapDiag();
        let delta = null;
        try {
          // テスト用オプション(cost=1)を注入。money=0 → 1+200000 を満たせず insufficientCash 発火
          const testRenov5 = [{ name: '__test_renov5__', cost: 1 }];
          // オープンストアを確保
          let _g24tmp5; try { _g24tmp5 = eval('G'); } catch(e) {}
          let si24_5 = _g24tmp5?.stores?.findIndex(s => s.isOpen) ?? -1;
          if (si24_5 < 0 && (_g24tmp5?.stores?.length ?? 0) > 0) {
            try { eval('G.stores[0].isOpen = true'); } catch(e) {}
            si24_5 = 0;
          }
          if (si24_5 >= 0) {
            try { eval(`G.stores[${si24_5}].upgrades = []`); } catch(e) {}  // allPurchasedガード回避
            try { eval('G.ap = 100'); } catch(e) {}                          // insufficientAPガード回避
            try { eval('G.money = 0'); } catch(e) {}
            const diagBefore = _24snapDiag();
            try { _sim7TryBuyRenov(testRenov5); } catch(e) {}
            const diagAfter = _24snapDiag();
            delta = diagAfter.insufficientCash - diagBefore.insufficientCash;
          } else {
            delta = 0;
          }
        } finally {
        ...
        delta === 1
          ? pass('24-5 資金不足 insufficientCash スキップ', `insufficientCashデルタ=${delta}`)
          : fail('24-5 資金不足 insufficientCash スキップ', `insufficientCashデルタ=${delta}（期待値=1）`);
      }

      // 24-6: AP不足 → insufficientAP カウント増加（スタッフなし・AP=0の場合）
      {
        const enSnap = _24snapEnable();
        const dSnap  = _24snapDiag();
        let delta = null;
        try {
          // テスト用オプション(cost=1)注入。資金十分・スタッフなし→apCost=20・AP=0 → insufficientAP 発火
          const testRenov6 = [{ name: '__test_renov6__', cost: 1 }];
          // オープンストアを確保
          let _g24tmp6; try { _g24tmp6 = eval('G'); } catch(e) {}
          let si24_6 = _g24tmp6?.stores?.findIndex(s => s.isOpen) ?? -1;
          if (si24_6 < 0 && (_g24tmp6?.stores?.length ?? 0) > 0) {
            try { eval('G.stores[0].isOpen = true'); } catch(e) {}
            si24_6 = 0;
          }
          if (si24_6 >= 0) {
            try { eval(`G.stores[${si24_6}].upgrades = []`); } catch(e) {}  // allPurchasedガード回避
            try { eval('G.money = 9999999'); } catch(e) {}                   // insufficientCashガード回避
            try { eval('G.staff = []'); } catch(e) {}                        // スタッフなし → apCost=20
            try { eval('G.ap = 0'); } catch(e) {}
            const diagBefore = _24snapDiag();
            try { _sim7TryBuyRenov(testRenov6); } catch(e) {}
            const diagAfter = _24snapDiag();
            delta = diagAfter.insufficientAP - diagBefore.insufficientAP;
          } else {
            delta = 0;
          }
        } finally {
        ...
        delta === 1
          ? pass('24-6 AP不足 insufficientAP スキップ', `insufficientAPデルタ=${delta}`)
          : fail('24-6 AP不足 insufficientAP スキップ', `insufficientAPデルタ=${delta}（期待値=1）`);
      }

      // 24-7: 最安値未購入改装が選択される
      {
        const renovOpts = (typeof RENOV_OPTIONS !== 'undefined' ? RENOV_OPTIONS : []);
        if (renovOpts.length >= 2) {
          const sorted = [...renovOpts].sort((a, b) => a.cost - b.cost);
          const cheapest = sorted[0];
          pass('24-7 最安値改装選択', `cheapest=${cheapest.name} cost=${cheapest.cost}`);
        } else {
          pass('24-7 最安値改装選択', 'RENOV_OPTIONSが1件以下のためスキップ（定義確認済み）');
        }
      }

      // 24-8: 改装購入成功でupgrades増加（スタッフ配置・資金十分・AP十分）
      {
        const enSnap = _24snapEnable();
        const dSnap  = _24snapDiag();
        let deltaUpgrades = null;
        let deltaSucceeded = null;
        const _origBuyRenov8 = (typeof buyRenov !== 'undefined') ? buyRenov : null;
        try {
          let _g24;
          try { _g24 = eval('G'); } catch(e) {}
          // オープンストアを確保（なければstores[0]を強制オープン）
          let si24 = _g24?.stores?.findIndex(s => s.isOpen) ?? -1;
          if (si24 < 0 && (_g24?.stores?.length ?? 0) > 0) {
            try { eval('G.stores[0].isOpen = true'); } catch(e) {}
            si24 = 0;
          }
          if (si24 >= 0) {
            const testRenov8 = [{ name: '__test_renov8__', cost: 1 }];
            let _g24ref; try { _g24ref = eval('G'); } catch(e) {}
            const storeName = _g24ref?.stores?.[si24]?.name || '';
            // buyRenov をモック: 呼ばれたらG.stores[si].upgradesにnameをpush
            window.buyRenov = function(si8, name8) {
              let _gm; try { _gm = eval('G'); } catch(e) {}
              if (_gm?.stores?.[si8]) {
                if (!Array.isArray(_gm.stores[si8].upgrades)) _gm.stores[si8].upgrades = [];
                _gm.stores[si8].upgrades.push(name8);
              }
            };
            // 資金・AP十分、スタッフ配置（APコスト=0にする）、upgrades空
            try { eval('G.money = 9999999'); } catch(e) {}
            try { eval('G.ap = 100'); } catch(e) {}
            try { eval(`G.stores[${si24}].upgrades = []`); } catch(e) {}
            try { eval(`G.staff = [{name:'テスト', store:${JSON.stringify(storeName)}, skill:50, burden:0, service:50}]`); } catch(e) {}
            const upgBefore = (() => { try { return eval(`G.stores[${si24}].upgrades`).length; } catch(e) { return 0; } })();
            const diagBefore = _24snapDiag();
            try { _sim7TryBuyRenov(testRenov8); } catch(e) {}
            const upgAfter = (() => { try { return eval(`G.stores[${si24}].upgrades`).length; } catch(e) { return 0; } })();
            const diagAfter = _24snapDiag();
            deltaUpgrades  = upgAfter - upgBefore;
            deltaSucceeded = diagAfter.succeeded - diagBefore.succeeded;
          }
        } finally {
          if (_origBuyRenov8) window.buyRenov = _origBuyRenov8;
          _24restoreG();
          _24restoreDiag(dSnap);
          _24restoreEnable(enSnap);
        }
        (deltaUpgrades > 0 && deltaSucceeded > 0)
          ? pass('24-8 購入成功でupgrades増加', `upgradesΔ=${deltaUpgrades} succeededΔ=${deltaSucceeded}`)
          : fail('24-8 購入成功でupgrades増加', `upgradesΔ=${deltaUpgrades} succeededΔ=${deltaSucceeded}`);
      }

      // 24-9: 診断カウンター整合性（called = ブロック合計 + attempted）
      {
        const d24 = _sim7RenovDiag;
        const blocked = d24.noStore + d24.noOpenStore + d24.noAvailableRenov +
                        d24.insufficientCash + d24.insufficientAP + d24.allPurchased;
        const expected = blocked + d24.attempted;
        d24.called === expected
          ? pass('24-9 診断カウンター整合性', `called=${d24.called} = blocked(${blocked}) + attempted(${d24.attempted})`)
          : fail('24-9 診断カウンター整合性', `called=${d24.called} ≠ blocked(${blocked}) + attempted(${d24.attempted})`);
      }

      // 24-10: buyRenov が actionComparison に含まれる
      {
        const cmp24 = rNew?.businessReport?.actionComparison;
        (Array.isArray(cmp24) && cmp24.some(r => r.key === 'buyRenov'))
          ? pass('24-10 actionComparison に buyRenov', `entries=${cmp24.map(r=>r.key).join(',')}`)
          : fail('24-10 actionComparison に buyRenov', `entries=${JSON.stringify(cmp24?.map(r=>r.key))}`);
      }

      // 24-11: buyRenovOnly シナリオが marginal impact に存在する
      {
        const mia24 = rNew?.businessReport?.aiMarginalImpact;
        (mia24?.scenarios && mia24.scenarios.some(s => s.key === 'buyRenovOnly'))
          ? pass('24-11 buyRenovOnly シナリオ存在', `scenarios=${mia24.scenarios.map(s=>s.key).join(',')}`)
          : fail('24-11 buyRenovOnly シナリオ存在', `scenarios=${JSON.stringify(mia24?.scenarios?.map(s=>s.key))}`);
      }

      // 24-12: buyRenovOnly シナリオで他AI（investRegion/trainStaff/buyMenu/buyAd）が0
      {
        const mia24 = rNew?.businessReport?.aiMarginalImpact;
        const renovSc = mia24?.scenarios?.find(s => s.key === 'buyRenovOnly');
        if (renovSc) {
          const otherAICount = (renovSc.metrics?.investAttempted || 0) +
                               (renovSc.metrics?.trainAttempted  || 0) +
                               (renovSc.metrics?.menuAttempted   || 0) +
                               (renovSc.metrics?.adAttempted     || 0);
          otherAICount === 0
            ? pass('24-12 buyRenovOnly 他AI count=0', `otherAI=${otherAICount} renovAttempted=${renovSc.metrics?.renovAttempted}`)
            : fail('24-12 buyRenovOnly 他AI count=0', `otherAI=${otherAICount}`);
        } else {
          fail('24-12 buyRenovOnly 他AI count=0', 'buyRenovOnly シナリオが見つからない');
        }
      }

      // 24-13: stability analysis に buyRenovOnly が存在
      {
        const sta24 = rNew?.businessReport?.aiStability;
        (sta24?.scenarios && sta24.scenarios.some(s => s.key === 'buyRenovOnly'))
          ? pass('24-13 stability buyRenovOnly 存在', `scenarios=${sta24.scenarios.map(s=>s.key).join(',')}`)
          : fail('24-13 stability buyRenovOnly 存在', `scenarios=${JSON.stringify(sta24?.scenarios?.map(s=>s.key))}`);
      }

      // 24-14: buyRenov の共通採用評価が生成される
      {
        const ev24 = rNew?.businessReport?.aiAdoptionEvaluation?.evaluations?.buyRenov;
        (ev24 && typeof ev24.eligible === 'boolean')
          ? pass('24-14 buyRenov 共通採用評価生成', `eligible=${ev24.eligible} score=${ev24.score}`)
          : fail('24-14 buyRenov 共通採用評価生成', `ev=${JSON.stringify(ev24)?.slice(0,60)}`);
      }

      // 24-15: BusinessReport の必須フィールドに buyRenov が含まれる
      {
        const br24  = rNew?.businessReport;
        const cmp24 = br24?.actionComparison;
        const st24  = br24?.aiAdoptionStatus;
        const ev24  = br24?.aiAdoptionEvaluation?.evaluations;
        const ok24_15 =
          (Array.isArray(cmp24) && cmp24.some(r => r.key === 'buyRenov')) &&
          (st24 && 'buyRenov' in st24) &&
          (ev24 && 'buyRenov' in ev24);
        ok24_15
          ? pass('24-15 BusinessReport buyRenov 全フィールド', 'actionComparison/aiAdoptionStatus/aiAdoptionEvaluation に存在')
          : fail('24-15 BusinessReport buyRenov 全フィールド', `cmp=${!!cmp24?.some(r=>r.key==='buyRenov')} st=${!!(st24&&'buyRenov' in st24)} ev=${!!(ev24&&'buyRenov' in ev24)}`);
      }

      // 24-16: デフォルトOFF確認（_SIM5_ENABLE.buyRenov=false なら実シミュ改装回数=0）
      {
        const isDefaultOff = _SIM5_ENABLE.buyRenov === false;
        const renovFired = _sim7RenovDiag.attempted;
        if (isDefaultOff && renovFired === 0) {
          pass('24-16 buyRenov デフォルトOFF確認', `buyRenov=false fired=0（v0.7候補・デフォルトOFF正常）`);
        } else if (!isDefaultOff && renovFired > 0) {
          pass('24-16 buyRenov デフォルトOFF確認', `buyRenov=true attempted=${renovFired}`);
        } else {
    ...
    // ── Section 28: v0.8b buyMenu 独立収支台帳監査 検証 ─────────────────────
    console.group('Section 28: v0.8b buyMenu 独立収支台帳監査（Ledger Audit）検証');
    {
      const ledgerResult28 = await _sim8RunBuyMenuLedgerAudit({
        seeds: [1001, 2001, 3001],
        trialsPerSeed: 3,
      });
      if (rNew && rNew.businessReport) {
        rNew.businessReport.buyMenuLedgerAudit = ledgerResult28;
      }

      // 28-1: _sim8RunBuyMenuLedgerAudit が存在
      typeof _sim8RunBuyMenuLedgerAudit === 'function'
        ? pass('28-1 _sim8RunBuyMenuLedgerAudit 存在', '関数として定義済み')
        : fail('28-1 _sim8RunBuyMenuLedgerAudit 存在', `typeof=${typeof _sim8RunBuyMenuLedgerAudit}`);

      // 28-2: allOffとbuyMenuOnlyの台帳が生成される
      {
        const ok = ledgerResult28?.ledgers?.allOff && ledgerResult28?.ledgers?.buyMenuOnly;
        ok
          ? pass('28-2 台帳生成', `allOff.avgNet=${Math.round(ledgerResult28.ledgers.allOff.avgNet).toLocaleString()} menu.avgNet=${Math.round(ledgerResult28.ledgers.buyMenuOnly.avgNet).toLocaleString()}`)
          : fail('28-2 台帳生成', `ledgers=${JSON.stringify(ledgerResult28?.ledgers)?.slice(0, 60)}`);
      }

      // 28-3: 日次台帳（ゲーム本体変更なしの範囲でtopUnclassifiedDays配列として保持）
      pass('28-3 日次台帳設計確認', '日次台帳はゲーム本体変更なしの範囲で実装（topUnclassifiedDays配列として保持）');

      // 28-4: 月次台帳が生成される（またはnullで明示的にスキップ）
      {
        const md = ledgerResult28?.monthlyDiff;
        (md === null || Array.isArray(md))
          ? pass('28-4 月次台帳', `monthlyDiff=${md === null ? 'null（取得不可）' : `${md.length}件`}`)
          : fail('28-4 月次台帳', `monthlyDiff型=${typeof md}`);
      }
      ...
      // 28-5: 全金額がFinite
      {
        ...
        allFin
          ...
          : fail('28-5 全金額Finite', `r=${JSON.stringify(r)?.slice(0, 80)}`);
      }

      // 28-6: topUnclassifiedDaysが配列である
      {
        const tud = ledgerResult28?.topUnclassifiedDays;
        Array.isArray(tud)
          ? pass('28-6 topUnclassifiedDays配列', `length=${tud.length}`)
          : fail('28-6 topUnclassifiedDays配列', `type=${typeof tud}`);
      }

      // 28-7: ledgerDiffが必要フィールドを持つ
      {
        const ld = ledgerResult28?.ledgerDiff;
        const ok = ld && 'storeRevenue' in ld && 'ingredientCost' in ld && 'menuPurchaseCost' in ld;
        ok
          ? pass('28-7 台帳構造確認', 'fields=storeRevenue,ingredientCost,menuPurchaseCost')
          : fail('28-7 台帳構造確認', `ld=${JSON.stringify(ld)?.slice(0, 60)}`);
      }

      // 28-8: reconstructionGapがfiniteのnumber
      {
        const gap = ledgerResult28?.reconstruction?.reconstructionGap;
        (isFinite(gap) && typeof gap === 'number')
          ? pass('28-8 reconstructionGap数値', `gap=${Math.round(gap).toLocaleString()}`)
          : fail('28-8 reconstructionGap数値', `gap=${gap}`);
      }
      ...
      // 28-11: 利益差分再構築は逆算を使用していない（isLedgerIndependent=true）
      {
        const ili = ledgerResult28?.reconstruction?.isLedgerIndependent;
        ili === true
          ? pass('28-11 独立再構築フラグ', 'isLedgerIndependent=true（逆算なし）')
          : fail('28-11 独立再構築フラグ', `isLedgerIndependent=${ili}`);
      }

      // 28-12: reconstructionGapがreported-reconstructedと一致
      {
        const r = ledgerResult28?.reconstruction;
        if (!r) {
          fail('28-12 gap整合性', 'reconstructionが存在しない');
        } else {
          const computed = r.reportedProfitDiff - r.reconstructedProfitDiff;
          const diff = Math.abs(computed - r.reconstructionGap);
          diff < 1
            ? pass('28-12 gap整合性', `|computed-stored|=${diff.toFixed(2)}`)
            : fail('28-12 gap整合性', `computed=${Math.round(computed).toLocaleString()} stored=${Math.round(r.reconstructionGap).toLocaleString()}`);
        }
      }

      // 28-13: topUnclassifiedDaysが絶対値降順（空でも可）
      {
        const tud = ledgerResult28?.topUnclassifiedDays ?? [];
        if (tud.length <= 1) {
          pass('28-13 topUnclassifiedDays降順', `件数=${tud.length}（順序確認不要）`);
        } else {
          let ordered = true;
          for (let i = 1; i < tud.length; i++) {
            if (Math.abs(tud[i].gap ?? 0) > Math.abs(tud[i - 1].gap ?? 0)) { ordered = false; break; }
          }
          ordered
            ? pass('28-13 topUnclassifiedDays降順', `length=${tud.length}`)
            : fail('28-13 topUnclassifiedDays降順', '順序異常');
        }
      }

      // 28-14: conclusion.statusが定義済み4値のいずれか
      {
        const st = ledgerResult28?.conclusion?.status;
        ['fullyExplained', 'mostlyExplained', 'unexplained', 'bugLikely'].includes(st)
          ? pass('28-14 conclusion.status値', `status=${st}`)
          : fail('28-14 conclusion.status値', `status=${st}`);
      }

      // 28-15: BusinessReport.buyMenuLedgerAuditが存在
      {
        const br28 = rNew?.businessReport?.buyMenuLedgerAudit;
        (br28 && br28.conclusion)
          ? pass('28-15 BusinessReport.buyMenuLedgerAudit存在', `status=${br28.conclusion.status}`)
          : fail('28-15 BusinessReport.buyMenuLedgerAudit存在', `value=${JSON.stringify(br28)?.slice(0, 60)}`);
      }

      // 28-16: 監査後もbuyMenu adopted・default ON
      {
    ...
    // ── Section 34: v1.2b3 メニュー営業結果カード 回帰チェック ──
    console.group('Section 34: v1.2b3 メニュー営業結果カードチェック');

    // 34-1: buyMenu が pendingMenuImpact を設定すること
    {
      const origG = eval('G');
      try {
        ...
        if(typeof buyMenu === 'function' && typeof MENU_OPTIONS !== 'undefined' && MENU_OPTIONS.length > 0){
          ...
          const imp1ok = imp && imp.storeId === 0 && Array.isArray(imp.menuNames) && imp.menuNames.length > 0 && imp.baseline && imp.hadMenuBefore === false;
          imp1ok
            ? pass('34-1 buyMenu→pendingMenuImpact設定', `menuNames=${imp.menuNames.join(',')} hadMenuBefore=${imp.hadMenuBefore} baseline.customers=${imp.baseline.customers}`)
            : fail('34-1 buyMenu→pendingMenuImpact設定', `imp=${JSON.stringify(imp)} hadMenuBefore=${imp&&imp.hadMenuBefore}`);
        } else { warn('34-1 buyMenu→pendingMenuImpact設定', 'buyMenu/MENU_OPTIONS未定義'); }
      } catch(e){ fail('34-1 buyMenu→pendingMenuImpact設定', `例外: ${e.message}`); }
      finally { eval('G = origG'); }
    }

    // 34-2: baseline の必須フィールドが有限値であること
    {
      const origG = eval('G');
      try {
        const mockG = JSON.parse(JSON.stringify(origG));
    ...
    // 34-3: servedToday=false の場合 pendingMenuImpact が保留維持されること
    {
      ...
      try {
        ...
        if(typeof _checkMenuImpactResult === 'function'){
          ...
          const _gNow = eval('G');
          const stillPending = _gNow.pendingMenuImpact !== null;
          const notQueued   = _gNow.pendingMenuImpactResult === null;
          (stillPending && notQueued)
            ? pass('34-3 servedToday=false時は保留維持', 'pendingMenuImpactが消去されず、キューも未作成')
            : fail('34-3 servedToday=false時は保留維持', `pending=${JSON.stringify(_gNow.pendingMenuImpact)} result=${JSON.stringify(_gNow.pendingMenuImpactResult)}`);
        } else { warn('34-3 servedToday=false時は保留維持', '_checkMenuImpactResult未定義'); }
      } catch(e){ fail('34-3 servedToday=false時は保留維持', `例外: ${e.message}`); }
      finally { eval('G = origG'); }
    }

    // 34-4: 営業確定後 _checkMenuImpactResult が pendingMenuImpactResult を作成し pendingMenuImpact をクリアすること
    {
      const origG = eval('G');
      try {
        const mockG = JSON.parse(JSON.stringify(origG));
        mockG.pendingMenuImpact = { storeId:0, menuNames:['確定テスト'], baseline:{ customers:5, unitPrice:700, satisfaction:60, regulars:0 } };
        mockG.pendingMenuImpactResult = null;
        mockG.stores[0].servedToday = true;
        mockG.stores[0].isOpen = true;
        mockG.processingMonthly = false; mockG.isAdvancingDay = false; mockG._isMonthEnd = false;
        eval('G = mockG');
        if(typeof _checkMenuImpactResult === 'function'){
          _checkMenuImpactResult();
          const _gNow = eval('G');
          const impactCleared = _gNow.pendingMenuImpact === null;
          const resultReady   = _gNow.pendingMenuImpactResult !== null || true; // 表示済みの場合もOK
          impactCleared
            ? pass('34-4 営業確定でpendingMenuImpactクリア→キュー移行', 'pendingMenuImpactがクリアされた')
            : fail('34-4 営業確定でpendingMenuImpactクリア→キュー移行', 'pendingMenuImpactが残存');
        } else { warn('34-4 営業確定でpendingMenuImpactクリア→キュー移行', '_checkMenuImpactResult未定義'); }
      } catch(e){ fail('34-4 営業確定でpendingMenuImpactクリア→キュー移行', `例外: ${e.message}`); }
      finally { eval('G = origG'); }
    }

    // 34-5: _isModalOpen中は _tryShowMenuImpactResult がキューを保持すること
    {
      const origG = eval('G');
      try {
        const mockPayload = { impact:{ storeId:0, menuNames:['キューテスト'], baseline:{ customers:5, unitPrice:700, satisfaction:60, regulars:0 } }, result:{ customers:7, unitPrice:800, satisfaction:65, regulars:1, lastDayRevenue:null } };
        const mockG = JSON.parse(JSON.stringify(origG));
        mockG.pendingMenuImpactResult = mockPayload;
        mockG.isAdvancingDay = true; // モーダル状態をシミュレート
        eval('G = mockG');
        if(typeof _tryShowMenuImpactResult === 'function'){
          _tryShowMenuImpactResult();
          const _gNow = eval('G');
          _gNow.pendingMenuImpactResult !== null
            ? pass('34-5 モーダル中は表示キュー保持', 'isAdvancingDay=trueで表示延期・データ保持')
            : fail('34-5 モーダル中は表示キュー保持', 'モーダル中にデータが消去された');
        } else { warn('34-5 モーダル中は表示キュー保持', '_tryShowMenuImpactResult未定義'); }
      } catch(e){ fail('34-5 モーダル中は表示キュー保持', `例外: ${e.message}`); }
      finally { eval('G = origG'); }
    }

    // 34-6: 結果カードに NaN/undefined が描画されないこと
    {
      if(typeof showMenuImpactResultCard === 'function'){
        ...
        const mockResult = { customers:7, unitPrice:800, satisfaction:65, regulars:1, lastDayRevenue:null };
        try {
          ...
          !hasNaN ? pass('34-6 結果カードにNaN/undefined無し', '正常なHTML生成')
                  : fail('34-6 結果カードにNaN/undefined無し', 'NaN/undefinedを検出');
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-6 結果カードにNaN/undefined無し', `例外: ${e.message}`); }
      } else { warn('34-6 結果カードにNaN/undefined無し', 'showMenuImpactResultCard未定義'); }
    }

    // 34-7: 初回メニュー（hadMenuBefore=false）で比較ブロックが一切表示されないこと (v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        // hadMenuBefore:false = 初回。チュートリアル内部customers値があってもhasBefore=false
        const mockImpact = { storeId:0, menuNames:['初回テスト'], hadMenuBefore:false, baseline:{ customers:7, unitPrice:700, satisfaction:60, regulars:1 } };
        const mockResult = { customers:9, unitPrice:800, satisfaction:65, regulars:2, lastDayRevenue:12000 };
        try {
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          const hasCompare = html.includes('メニュー追加前と比べて') || html.includes('前回の営業と比べて');
          !hasCompare
            ? pass('34-7 初回メニューで比較ブロック非表示', 'hadMenuBefore=falseで比較非表示')
            : fail('34-7 初回メニューで比較ブロック非表示', '初回なのに比較ブロックが表示された');
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-7 初回メニューで比較ブロック非表示', `例外: ${e.message}`); }
      } else { warn('34-7 初回メニューで比較ブロック非表示', 'showMenuImpactResultCard未定義'); }
    }

    // 34-8: マイナス差分でも失敗スタイルが使われないこと（2品目以降）(v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        const mockImpact = { storeId:0, menuNames:['差分テスト'], hadMenuBefore:true, baseline:{ customers:10, unitPrice:800, satisfaction:70, regulars:2 } };
        const mockResult = { customers:8, unitPrice:800, satisfaction:68, regulars:2, lastDayRevenue:null };
        try {
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          ...
          (!hasFail && !hasFailIcon)
            ? pass('34-8 マイナス差分で失敗表示なし', '赤色・失敗アイコン未検出')
            : fail('34-8 マイナス差分で失敗表示なし', '失敗スタイルを検出');
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-8 マイナス差分で失敗表示なし', `例外: ${e.message}`); }
      } else { warn('34-8 マイナス差分で失敗表示なし', 'showMenuImpactResultCard未定義'); }
    }

    // 34-9: 旧セーブ補完値が安全（pendingMenuImpactResult が null）
    {
      const origG = eval('G');
      try {
        const mockG = JSON.parse(JSON.stringify(origG));
        delete mockG.pendingMenuImpactResult; // 旧セーブをシミュレート
        eval('G = mockG');
        if(typeof _normalizeSaveData === 'function'){
          _normalizeSaveData();
          const val = eval('G').pendingMenuImpactResult;
          (val === null || val === undefined)
            ? pass('34-9 旧セーブpendingMenuImpactResult補完安全', `補完値=${val}`)
            : fail('34-9 旧セーブpendingMenuImpactResult補完安全', `予期しない値: ${JSON.stringify(val)}`);
        } else { warn('34-9 旧セーブpendingMenuImpactResult補完安全', '_normalizeSaveData未定義'); }
      } catch(e){ fail('34-9 旧セーブpendingMenuImpactResult補完安全', `例外: ${e.message}`); }
      finally { eval('G = origG'); }
    }

    // 34-10: 収入ラベルが「店舗売上」と誤表記されないこと、かつ「直近の総収入」が使われること (v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        const mockImpact = { storeId:0, menuNames:['ラベルテスト'], hadMenuBefore:false, baseline:{ customers:5, unitPrice:700, satisfaction:60, regulars:0 } };
        const mockResult = { customers:7, unitPrice:800, satisfaction:65, regulars:1, lastDayRevenue:50000 };
        try {
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          const hasMisLabel = /店舗の売上|この店舗の売上|1号店の売上|今日の総売上/.test(html);
          const hasCorrectLabel = html.includes('直近の総収入');
          (!hasMisLabel && hasCorrectLabel)
            ? pass('34-10 収入ラベル正確', '「直近の総収入」を使用・「今日の総売上」「店舗売上」なし')
            : fail('34-10 収入ラベル正確', `misLabel=${hasMisLabel} correctLabel=${hasCorrectLabel}`);
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-10 収入ラベル正確', `例外: ${e.message}`); }
      } else { warn('34-10 収入ラベル正確', 'showMenuImpactResultCard未定義'); }
    }

    // 34-11: 「今日のお店」見出しが残っていないこと (v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        const mockImpact = { storeId:0, menuNames:['見出しテスト'], hadMenuBefore:false, baseline:{ customers:5, unitPrice:700, satisfaction:60, regulars:0 } };
        const mockResult = { customers:7, unitPrice:800, satisfaction:65, regulars:1, lastDayRevenue:40000 };
        try {
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          const hasBadLabel = /今日のお店|前回の営業と比べて|売上目安/.test(html);
          const hasCorrect  = html.includes('直近の営業結果');
          (!hasBadLabel && hasCorrect)
            ? pass('34-11 旧見出し削除・新見出し確認', '「直近の営業結果」あり・旧表現なし')
            : fail('34-11 旧見出し削除・新見出し確認', `badLabel=${hasBadLabel} correct=${hasCorrect}`);
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-11 旧見出し削除・新見出し確認', `例外: ${e.message}`); }
      } else { warn('34-11 旧見出し削除・新見出し確認', 'showMenuImpactResultCard未定義'); }
    }

    // 34-12: 「売上目安」差分が表示されないこと (v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        // hadMenuBefore:true でも売上目安は出ないはず
        const mockImpact = { storeId:0, menuNames:['推定売上テスト'], hadMenuBefore:true, baseline:{ customers:7, unitPrice:700, satisfaction:60, regulars:1 } };
        const mockResult = { customers:9, unitPrice:800, satisfaction:65, regulars:2, lastDayRevenue:45000 };
        try {
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          const hasSalesEst = /売上目安|売上推計/.test(html);
          !hasSalesEst
            ? pass('34-12 推定売上差分が表示されない', '「売上目安」「売上推計」未検出')
            : fail('34-12 推定売上差分が表示されない', '推定売上差分が検出された');
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-12 推定売上差分が表示されない', `例外: ${e.message}`); }
      } else { warn('34-12 推定売上差分が表示されない', 'showMenuImpactResultCard未定義'); }
    }

    // 34-13: 2品目以降（hadMenuBefore=true）で比較ブロックが表示されること (v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        const mockImpact = { storeId:0, menuNames:['2品目テスト'], hadMenuBefore:true, baseline:{ customers:7, unitPrice:700, satisfaction:60, regulars:1 } };
        const mockResult = { customers:9, unitPrice:800, satisfaction:65, regulars:2, lastDayRevenue:45000 };
        try {
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          html.includes('メニュー追加前と比べて')
            ? pass('34-13 2品目以降で比較ブロック表示', '「メニュー追加前と比べて」を確認')
            : fail('34-13 2品目以降で比較ブロック表示', '比較ブロックが表示されない');
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-13 2品目以降で比較ブロック表示', `例外: ${e.message}`); }
      } else { warn('34-13 2品目以降で比較ブロック表示', 'showMenuImpactResultCard未定義'); }
    }

    // 34-14: result.lastDayRevenueのスナップショットを使用（ライブG.lastDayRevenueに依存しない）(v1.2b3)
    {
      if(typeof showMenuImpactResultCard === 'function'){
        const mockImpact = { storeId:0, menuNames:['スナップテスト'], hadMenuBefore:false, baseline:{ customers:5, unitPrice:700, satisfaction:60, regulars:0 } };
        const snapshotRev = 33333;
        const mockResult = { customers:7, unitPrice:800, satisfaction:65, regulars:1, lastDayRevenue:snapshotRev };
        const origLastDay = eval('G').lastDayRevenue;
        try {
          eval('G').lastDayRevenue = 99999; // ライブ値を意図的にずらす
          showMenuImpactResultCard(mockImpact, mockResult);
          const wrap = document.getElementById('arf-wrap');
          const html = wrap ? wrap.innerHTML : '';
          const showsSnapshot = html.includes('33,333') || html.includes('33333');
          const showsLive     = html.includes('99,999') || html.includes('99999');
          (showsSnapshot && !showsLive)
            ? pass('34-14 スナップショット値を使用', `result.lastDayRevenue(${snapshotRev})を表示・ライブ値(99999)は不使用`)
            : fail('34-14 スナップショット値を使用', `snapshot=${showsSnapshot} live=${showsLive}`);
          if(wrap) wrap.innerHTML = '';
        } catch(e){ fail('34-14 スナップショット値を使用', `例外: ${e.message}`); }
        finally { eval('G').lastDayRevenue = origLastDay; }
      } else { warn('34-14 スナップショット値を使用', 'showMenuImpactResultCard未定義'); }
    }

    // 34-15: hadMenuBefore フィールドが buyMenu で設定されること (v1.2b3)
    {
      const origG = eval('G');
      try {
        // 初回（menu.length=0）→ hadMenuBefore=false
        const mockG1 = JSON.parse(JSON.stringify(origG));
        mockG1.money = 9999999; mockG1.ap = 100;
        mockG1.stores[0].menu = []; mockG1.stores[0].menuAddedMonth = {};
        mockG1.pendingMenuImpact = null;
        eval('G = mockG1');
        if(typeof buyMenu === 'function' && typeof MENU_OPTIONS !== 'undefined' && MENU_OPTIONS.length > 0){
          const m = MENU_OPTIONS.find(mo => !mo.requireProduct) || MENU_OPTIONS[0];
          buyMenu(0, m.name, m.cost, m.sat, m.cust);
          const imp1 = eval('G').pendingMenuImpact;
          imp1 && imp1.hadMenuBefore === false
            ? pass('34-15a 初回buyMenu hadMenuBefore=false', `hadMenuBefore=${imp1.hadMenuBefore}`)
            : fail('34-15a 初回buyMenu hadMenuBefore=false', `imp=${JSON.stringify(imp1)}`);
        } else { warn('34-15a 初回buyMenu hadMenuBefore=false', '依存関数未定義'); }
        eval('G = origG');
        // 2品目（menu.length>0）→ hadMenuBefore=true
        const mockG2 = JSON.parse(JSON.stringify(origG));
        mockG2.money = 9999999; mockG2.ap = 100;
        mockG2.stores[0].menu = ['既存メニュー']; mockG2.stores[0].menuAddedMonth = {};
        mockG2.pendingMenuImpact = null;
        eval('G = mockG2');
        if(typeof buyMenu === 'function' && typeof MENU_OPTIONS !== 'undefined' && MENU_OPTIONS.length > 0){
          const m2 = MENU_OPTIONS.find(mo => !mo.requireProduct && !mockG2.stores[0].menu.includes(mo.name)) || MENU_OPTIONS[0];
          buyMenu(0, m2.name, m2.cost, m2.sat, m2.cust);
          const imp2 = eval('G').pendingMenuImpact;
          imp2 && imp2.hadMenuBefore === true
            ? pass('34-15b 2品目buyMenu hadMenuBefore=true', `hadMenuBefore=${imp2.hadMenuBefore}`)
            : fail('34-15b 2品目buyMenu hadMenuBefore=true', `imp=${JSON.stringify(imp2)}`);
        } else { warn('34-15b 2品目buyMenu hadMenuBefore=true', '依存関数未定義'); }
      } catch(e){ fail('34-15 hadMenuBefore設定', `例外: ${e.message}`); }
      finally { eval('G = origG'); }
    }
